...
const saveArrangedTasks = () => {
  ...
  const orderedIds = [...document.querySelectorAll(".task")].map((el) =>
    Number(el.dataset.id)
  );
  orderedIds.forEach((el) => console.log(el));
  const arranged = orderedIds.map((id) => tasks.find((t) => t.id === id));
  arranged.forEach((el) => console.log(el));
