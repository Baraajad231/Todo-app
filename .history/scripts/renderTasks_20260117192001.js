...
import { activeClassHandler } from "./filtersHandler.js";
import { getFilterState, getTasks, getTheme } from "./tasksStorage.js";

export const itemsCountHandler = (tasks) => {
  let count = 0;
  tasks.forEach((task) => {
    if (!task.isCompleted) {
      count++;
...
export const renderTasks = (type) => {
  ...
  tasks.forEach((task, index) => {
    ...
    }" data-index="${index}" data-completed="${task.isCompleted}" data-id="${
      task.id
    }">
