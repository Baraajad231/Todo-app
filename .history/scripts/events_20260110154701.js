...
export const initClickEventsListeners = () => {
  todo.addEventListener("click", (e) => {
    console.log("hello from delegation listener 1");

    const taskEl = e.target.closest(".task");
    if (taskEl) {
      console.log("hello from delegation listener 2");
    }
    if (taskEl && e.target.matches(".checkbox__icon")) {
      const index = parseInt(taskEl.dataset.index);
      checkboxHandler(index);
      console.log("hello from delegation listener 3");
    }

    if (e.target.matches(".todo__input .checkbox__icon")) {
      checkboxinputHandler();
    }
  });
};
