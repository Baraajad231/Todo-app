...
export const toggleThemeHandler = async () => {
  ...
  icon.classList.toggle("anim-out", icon.classList.contains("anim-in"));
  icon.classList.toggle("anim-in", icon.classList.contains("anim-out"));

  setTimeout(() => updateBackground(html.dataset.theme), 100);
  setTimeout(() => updateThemeIcon(html.dataset.theme), 1000);
