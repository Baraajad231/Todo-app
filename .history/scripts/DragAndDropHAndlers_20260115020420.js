...
const saveArrangedTasks = (dragged, next) => {
  ...
  // إيجاد مواقع العنصر المسحوب والعنصر الهدف
  const from = tasks.findIndex((t) => t.id === draggedId);
  const to = tasks.findIndex((t) => t.id === nextId);

  if (from === -1 || to === -1) return;

  // إزالة العنصر من مكانه الأصلي
  const [moved] = tasks.splice(from, 1);

  // وضع العنصر بعد العنصر الهدف مباشرة
  let insertIndex = to;
  if (from < to) {
    // إذا سحبنا العنصر لأعلى → التغيير في index بعد الحذف
    insertIndex = to;
  } else {
    // إذا سحبنا العنصر لأسفل → التغيير في index بعد الحذف
    insertIndex = to + 1;
  }
